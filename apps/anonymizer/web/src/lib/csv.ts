...
function decodeContent(buffer: ArrayBuffer): string {
  ...
  try {
    ...
    const misdecodedSequences = text.match(/[ÃÂ][\u0080-\u00BF]/g) ?? [];
    if (misdecodedSequences.length > 0 && misdecodedSequences.length / Math.max(text.length, 1) > 0.01) {
      ...
      Papa.parse(content, {
        ...
        complete: (results) => {
          const rows = (results.data as unknown[])
            ...
            .map((row) => row.map((v) => normalizeValue(v)));
