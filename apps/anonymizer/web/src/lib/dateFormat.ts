...
export function parseDateWithFormat(value: string, format: string): Date | null {
  ...
  if (date.getFullYear() !== parts.year) return null;
  if (parts.month !== undefined && date.getMonth() !== parts.month - 1) return null;
  if (parts.day !== undefined && date.getDate() !== parts.day) return null;
  if (parts.hour !== undefined && date.getHours() !== parts.hour) return null;
  if (parts.minute !== undefined && date.getMinutes() !== parts.minute) return null;
  if (parts.second !== undefined && date.getSeconds() !== parts.second) return null;
  if (parts.millisecond !== undefined && date.getMilliseconds() !== parts.millisecond) return null;
